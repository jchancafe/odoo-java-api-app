...
public class ObjectAdapter {
	...
	synchronized void validateModelExists() throws OdooApiException, XmlRpcException {
		...
		if (objectNameCache.indexOf(modelName) < 0) {
			...
			if (!isUserPortal()) {
				...
				System.err.println("This user seems to be a portal user. No control is done on the validity. Use it at your own risks!");
				objectNameCache.add(modelName);
			}
	...
	/**
	 * Prepares a ROW object to be used for setting values in import/write methods.
	 * This method calls to the server to get the fieldCollection. Use
	 * getNewRow(FieldCollection fields) if you can to reduce the number of calls to
	 * the server for a bulk load.
	 *
	 * @param fields Fields that should be included in the row definition
	 * @return An empty row with the specified fields
	 * @throws XmlRpcException
	...
	public RowCollection readObject(Object[] ids, String[] fields) throws XmlRpcException, OdooApiException {
		...
		/**
		 * ** 18/04/2012 - PvdM Maybe reconsider this piece of code for later. Does it
		 * matter if it isn't sorted by ID?
		 *
		 * // Odoo doesn't use the sorting you pass (specified in the search function to
		 * get a sorted list of IDs). // When they fix it, remove this section of code
		 * ArrayList<Integer> idList = new ArrayList <Integer>(); for (Object id : ids){
		 * idList.add(Integer.parseInt(id.toString())); } Object[] sortedResults = new
		 * Object[ids.length]; for (Object result : results){
		 *
		 * @SuppressWarnings("unchecked") int id = null null null null null null
	 ...
	 * @return A validated filter Object[] correctly formatted for use by the Odoo
	 *         search function
	 * @throws OdooApiException
	 */
	...
	private Object[] fixImportData(Row inputRow) throws OdooApiException, XmlRpcException {
		...
		for (int i = 0; i < inputRow.getFields().size(); i++) {
			...
			case MANY2MANY:
				/*
				 * The import function uses the Names of the objects for the import. Replace the
				 * ID list passed in with a Name list for the import_data function that we are
				 * about to call
				 */
	...
	/**
	 * Calls the import_data or load function on the server to bulk create/update
	 * records.
	 *
	 * The import_data function will be called on Odoo servers where the version
	 * number is < 7. The import_data function does not return IDs and therefore IDs
	 * will not be set on imported rows.
	 *
	...
	/**
	 * * Combines the searchObject and readObject calls. Allows for easy read of all
	 * data
	 *
	 * @param filter A filter collection that contains a list of filters to be
	 *               applied
	...
	/**
	 * Combines the searchObject and readObject calls and returns rows in batches.
	 * Useful for multi-threaded ETL applications.
	 *
	 * @param filter A filter collection that contains a list of filters to be
	 *               applied
	 * @param fields List of fields to return data for
	...
	/**
	 * Writes a collection of rows to the database by calling the write function on
	 * the object the Row is holding data for
	 *
	 * @param rows        Row collection to submit
	 * @param changesOnly Only changed values will be submitted to the database.
	 * @return An array of logicals. One for each row to indicate if the update was
	 *         successful
	 * @throws OdooApiException
	 * @throws XmlRpcException
	 */
	...
	/**
	 * Writes a Row to the database by calling the write function on the object the
	 * Row is holding data for
	 *
	...
	/**
	 * Creates an Object on the Odoo server by calling the create function on the
	 * server. The id column is set on the row after the object was successfully
	 * created
	 *
	...
	/**
	 * Calls any function on an object that returns a field collection. ie. a row is
	 * retured as [{'name' : {'type' : 'char'}] The Odoo function must have the
	 * signature like (self, cr, uid, *param).
	 *
	...
	/**
	 * Calls any function on an object. The first row is inspected to determine data
	 * fields and data types The Odoo function must have the signature like (self,
	 * cr, uid, *param) and return a dictionary or object.
	 *
	 * @param functionName function to call
	 * @param parameters   Additional parameters that will be passed to the object
	 * @param fieldCol     An option field collection to use. A new one will be
	 *                     built by inspecting the first row if it isn't specified
	 *                     (null).
