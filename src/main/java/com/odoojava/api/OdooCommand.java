...
public class OdooCommand {
    ...
    public Response searchObject(String objectName, String order, Object[] filter) throws XmlRpcException {
        return (Response) searchObject(objectName, filter, -1, -1, order, false);
    }

    /**
     * Searches for objects that satisfies the filter. These IDs are typically used
     * in a following readObject call to the server to get the data
     *
    ...
    public Object[] readObject(String objectName, Object[] ids, String[] fields) throws XmlRpcException {
        Object[] readResult;
        if (this.session.getServerVersion().getMajor() >= 8 && this.session.getServerVersion().getMajor() < 10) {
            ...
            readResult = (Object[]) session.executeCommandWithContext(objectName, "read",
                    new Object[] { ids, fields, new String[] {} });
        } else {
            // TODO: Have to be rewritten/deleted considering the previous call
            readResult = (Object[]) session.executeCommand(objectName, "read",
                    new Object[] { ids, fields, session.getContext() });
        }
     ...
     *                   specific naming conventions. Consider using the
     *                   ObjectAdapter
    ...
    public Object[] importData(String objectName, String[] fieldList, Object[][] rows) throws XmlRpcException {
        ...
        if (session.getServerVersion().getMajor() <= 9) {
            return (Object[]) session.executeCommand(objectName, "import_data",
     ...
     * @return An Object[] with an entry for each ID. Each entry is another Object
     *         [] with index 0 being the ID and index 1 being the Name
     * @throws XmlRpcException
     */
    ...
    /**
     * Calls any function on an object. The function Odoo must have the signature
     * like (self, cr, uid, *param) and return a dictionary or object. *param can be
     * replaced by separate parameters if you are sure of the number of parameters
     * expected
     *
